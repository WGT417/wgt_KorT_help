...
LABEL_FIXES={
    ...
    # 어미와 접사는 붙임표를 달고 조사는 달지 않는 것이 이 책들의 관습이다
    # (한국어문법총론 1 102: "의존 형태소를 표시하기 위해 흔히 붙임 표(hyphen)를
    # 사용한다 ... 조사도 의존 형태소이므로 ... 관습적으로 사용하지 않으므로";
    # 한국어표준문법 215 같은 취지). 찾아보기 표제는 그 붙임표를 떼고 실린 것이
    # 많아 카드 이름만 보면 어미인지 알 수 없다. 선어말어미는 양쪽에 단다.
    '(으)냐':'-(으)냐','(으)니까':'-(으)니까','(으)라고':'-(으)라고','(으)러':'-(으)러','(으)려고':'-(으)려고',
    '(으)려니와':'-(으)려니와','(으)려마':'-(으)려마','(으)려무나':'-(으)려무나','(으)렴':'-(으)렴','(으)마':'-(으)마',
    '(으)며':'-(으)며','(으)면':'-(으)면','(으)면서':'-(으)면서','(으)므로':'-(으)므로','(으)오':'-(으)오',
    '(으)이':'-(으)이','거니와':'-거니와','거 든':'-거든','거 라':'-거라','건대':'-건대','게(요)':'-게(요)','게나':'-게나',
    '고자':'-고자','구려':'-구려','군(요)':'-군(요)','까(요)':'-까(요)','나(요)':'-나(요)','냐고':'-냐고','너 라':'-너라',
    '네(요)':'-네(요)','느냐':'-느냐','느라고':'-느라고','는데':'-는데','다고':'-다고','다시피':'-다시피','단다':'-단다',
    '더냐':'-더냐','더라도':'-더라도','도록':'-도록','듯이':'-듯이','로구나':'-로구나','망정':'-망정','습니다':'-습니다',
    '어(요)':'-어(요)','어/아도':'-어/아도','어/아야':'-어/아야','아/어(요)':'-아/어(요)','아라/어라':'-아라/어라',
    '었다가':'-었다가','자고':'-자고','자마자':'-자마자','지(요)':'-지(요)','지만':'-지만','(으)시':'-(으)시-',
    '(으)리':'-(으)리-','사오':'-사오-','시옵':'-시옵-','스럽':'-스럽-','적(的)':'-적(的)','애/에':'-애/에',
    '무(無)':'무(無)-','미(未)':'미(未)-','불(不)':'불(不)-','생(生)':'생(生)-','친(親)':'친(親)-',
    # 한 음절을 닮은꼴로 읽은 이름. 같은 말이 다른 쪽에서는 제대로 찍혀 있어
    # 본문 빈도로 가려낸다(브레인스토멍 6회 : 브레인스토밍 36회).
    '브레인스토멍':'브레인스토밍','국힌문체':'국한문체','풍사전성':'품사 전성','이동문학':'아동문학','문학적상정':'문학적 상징',
    '이 역행동회':'ㅣ 역행 동화','통사구성이 어휘':'통사 구성의 어휘화','(으)님시오':'-(으)ㅂ시오','느냐! (으)냐':'-느냐/-(으)냐',
    # 찾아보기 줄에서 어절이 갈라진 이름. 책 본문이 그 말을 어떻게 띄어 쓰는지로
    # 되붙인다(‘귀의 성’처럼 띄어쓰기가 본디 그런 제목은 그대로 둔다).
    '어 말어 미':'어말 어미','의 미 구성':'의미 구성','평서 문':'평서문','존비 법':'존비법','직 설법':'직설법','과거 시 제':'과거 시제',
    '성 분부사':'성분 부사','눈동자 움직 임':'눈동자 움직임','기 행문':'기행문','정 의 요인':'정의 요인','핵심 역 량':'핵심 역량',
    '의 미 역':'의미역','미 래 시 제':'미래 시제','시 간성':'시간성','접 속부사':'접속 부사','접 두파생 법':'접두 파생법',
    '장기 기 억':'장기 기억','회귀 적':'회귀적','접 미 파생 법':'접미 파생법','허 락법':'허락법','수식 언':'수식언',
    '직 접 구성 성 분':'직접 구성 성분','지 시 부사':'지시 부사','부속성 분':'부속 성분','작업 기 억':'작업 기억',
    '어 린이 독자':'어린이 독자','유정 명 사':'유정 명사','4원적 체 계':'4원적 체계','호격 어':'호격어','( 이 ) 랑':'(이)랑',
    '( 이 )야':'(이)야','순수 서 정 시':'순수 서정시','두 자리 서 술어':'두 자리 서술어','범용어 미':'범용 어미',
    '구조적 아이 러 니':'구조적 아이러니','대 칭 동사':'대칭 동사','교육 정 전':'교육 정전','생 활양식':'생활 양식',
    '근접 발달 영 역':'근접 발달 영역','비 (非)':'비(非)-','비 판적 담론':'비판적 담론','영 역 특수적 성격':'영역 특수적 성격',
    '삼일치 법 칙':'삼일치 법칙','씨 (民)':'씨','조사연속구성 체':'조사 연속 구성체','상보적 인 분포':'상보적인 분포',
    '비 대격동사':'비대격 동사','영 변화파생':'영 변화 파생','외 적 구조':'외적 구조','정중어 법':'정중 어법','감정 적 플롯':'감정적 플롯',
    '성 별 특성':'성별 특성','시 감상기록장':'시 감상 기록장','작가의 사회 학':'작가의 사회학','이 다':'이다',
    # 카드를 열어 보고 찾은 나머지 오독. 알파 뱃·웅 집성·지음은 책 본문도 같은 꼴로
    # 찍혀 있으나 ㅔ/ㅐ, ㅡ/ㅜ, ㅏ/ㅣ를 맞바꾼 자리이고, 뒤의 넷은 괄호 안 원어와 한자가
    # 무너진 것이라 괄호를 떼거나 본문이 성한 쪽을 따른다.
    '알파뱃':'알파벳','웅집':'응집','지음':'자음','시 장착의 주된 모티브':'시 창작의 주된 모티브',
    '문맹( ill ite racy)':'문맹(illiteracy)','책맹(a lite racy)':'책맹','두운{頭觸':'두운',
    '죽은 비 유(dead metaphor)':'죽은 비유(dead metaphor)',
    # 카드 4,980장의 풀이를 이름과 함께 읽어 내려가며 더 찾은 것. 상정→상징, 시동사→사동사,
    # 이미앓→이미 앎은 책 본문이 같은 쪽에서 제 꼴을 함께 보여 준다(우리말문법론 6의 ‘직판’은
    # 책이 직관과 구별해 쓰는 말이라 그대로 둔다).
    '상정(symbol)':'상징(symbol)','상정사':'상징사','상정주의의 순수시':'상징주의의 순수시',
    '시동사':'사동사','이미앓':'이미 앎','화용론적특정':'화용론적 특성',
    '예시(據示)':'예시','세책賞冊':'세책','언문(꿇文)':'언문','인정기술A 定記述':'인정 기술',
    # 괄호 안 원어가 글자 사이에서 갈라진 이름. 띄어쓰기는 key_of가 지우므로 카드가 갈리지는
    # 않지만 화면에 그대로 보인다(미시 구조(m icro- structure) 같은 꼴).
    '미시 구조(m icro- structure)':'미시 구조(micro-structure)','북스타트( book sta rt)':'북스타트(book start)',
    '상징계(th e symbolic)':'상징계(the symbolic)','스탕달(Stendh a l)':'스탕달(Stendhal)',
    '아상블뢰즈(assambleu se)':'아상블뢰즈(assambleuse)','에우리피데스(E uripid es)':'에우리피데스(Euripides)',
    '정보 제공(i nformat ive) 기능':'정보 제공(informative) 기능','주동자(protagoni s t)':'주동자(protagonist)',
    '통제적(regu l atory) 기능':'통제적(regulatory) 기능','심 층적 역 설(depth paradox)':'심층적 역설(depth paradox)',
    '표층적 역 설(surface paradox)':'표층적 역설(surface paradox)',
    # 낱글자가 아주 무너진 이름. 책 본문이 그 자리를 말로 풀어 주는 것만 되살린다
    # (한국어표준문법 189쪽 “어미의 첫 모음 ‘어’가 탈락하는 현상”). 강도몽유록은 한자 병기가
    # 무너져 제목만 남긴다.
    '어미 초‘} ’탈락':'어미 초 ‘어’ 탈락','<강도 몽유 록(江都夢 낸 흉풍':'〈강도몽유록〉',
    # 어미·접사·보조 용언 구성의 붙임표. 찾아보기 줄은 붙임표를 자주 빠뜨리지만 책 본문이
    # 그 형태를 인용할 때는 붙임표를 붙여 쓴다(‘-구나’ 23회 : ‘구나’ 2회). 본문이 붙여 쓰는
    # 것만 되살리고, 붙임표가 한 번도 안 붙는 것((으)십시다·게요·네요)은 그대로 둔다.
    '거나':'-거나','건만':'-건만','구나':'-구나','구먼':'-구먼','구먼요':'-구먼요','군요':'-군요',
    '느니라':'-느니라','는구나':'-는구나','는지':'-는지','려무나':'-려무나','올시다':'-올시다','지요':'-지요',
    '다가':'-다가','어야':'-어야','았었':'-았었-','쟁이':'-쟁이','장이':'-장이','꾸러기':'-꾸러기','고서':'-고서',
    '았/었':'-았/었-','(으)니':'-(으)니','(으)나':'-(으)나','(으)리다':'-(으)리다','(으)소서':'-(으)소서',
    '(으)오리다':'-(으)오리다','(으)락 - -(으)락':'-(으)락 -(으)락','어라/아라':'-어라/아라','아요/어요':'-아요/어요',
    '(으/느)냐':'-(으/느)냐','아(서) /어(서)':'-아(서)/어(서)','아(요)/어 (요)':'-아(요)/어(요)',
    '는 것':'-는 것','는 중이다':'-는 중이다','기 시작하다':'-기 시작하다','게 되다':'-게 되다',
    '지 못하다':'-지 못하다','고 있다':'-고 있다','고 있':'-고 있-',
    # ㄴ을 알파벳 L로, ㄹ을 ‘근’으로 읽은 자리도 함께 되살린다(한국어표준문법 526쪽이
    # ‘-(으)ㄴ가(요)’로, 476쪽이 연결 어미 ‘-(으)ㄴ데’로 찍는다).
    '(으) L':'-(으)ㄴ','(느) L':'-(느)ㄴ','(으)L가':'-(으)ㄴ가','(으)L가요':'-(으)ㄴ가요',
    '(으)L 가(요)':'-(으)ㄴ가(요)','(으) L 것':'-(으)ㄴ 것','(으)L 데':'-(으)ㄴ데','(으)L 들':'-(으)ㄴ들',
    '(으)근 것':'-(으)ㄹ 것','는/L 다':'-는/ㄴ다',
    # 보조 용언 구성은 붙임표가 괄호 안에 든다: 우리말문법론 114쪽이 ‘(-어/아) 가다’로 찍는다.
    '( 어/아) 가다':'(-어/아) 가다','( 어/아) 가지 다':'(-어/아) 가지다','( 어/아) 두다':'(-어/아) 두다',
    '( 어/아) 보이 다':'(-어/아) 보이다',
    # 이름이 무너진 자리. 현대소설교육론은 같은 작가를 다른 쪽에서 ‘윤흥길’로 찍는다(238·324쪽
    # 윤 홍 길 : 117·287·388쪽 윤흥길).
    '여 기':'여기','윤홍길':'윤흥길',
}
...
JUNK={'찾아보기','색인','인영색인','잦m보기','증에}보기','찾아5ê기','칭i버{보기',
      ...
      # 찾아보기 줄이 아니라 책의 활동 제목·예문·본문 한 줄이 표제로 긁힌 것. 고전산문교육론의
      # ‘꼼꼼히 읽기: …’는 절 제목이고(현대소설교육론의 용어 ‘꼼꼼히 읽기’는 카드로 남긴다),
      # 나머지는 문장 한 도막이라 용어가 아니다.
      '57년의 발표 작으로는 한국 전쟁을 맞아 여러 가지 비극적', '<이춘풍전>의 세태 소설적 특성을 서술하여 보자', '@ 용죠::7} 마을에 남다', '@ 인류의 시원',
      'oS 몽 유 ㆍ완 유 소설:: 원생 몽유 록', 'oo BS Se: 심청전 춘향전 EA 홍 부전', '“황금인데 백 년 동안 부자로 살 수 있습니다"',
      '꼼꼼히 읽 71: 공간 구조와 그 의미', '꼼꼼히 읽 71: 구원의 공간인 피화 당(避網堂', '꼼꼼히 읽 71: 바리공주의 영웅적 면모', '꼼꼼히 읽기 ’ 풍자의 대상',
      '꼼꼼히 읽기. 사도셰자의마지막오습', '꼼꼼히 읽기. 천상계와 지상계의 미분리', '꼼꼼히 읽기: ‘ 십장가의 의미', '꼼꼼히 읽기: 감으I(感義',
      '꼼꼼히 읽기: 공간에 따른 인물 간의 대립', '꼼꼼히 읽기: 놀부 박 사설에 담긴 의미', '꼼꼼히 읽기: 심청의 윤리관', '꼼꼼히 읽기: 연암의 열녀 관',
      '꼼꼼히 읽기: 영웅의 일생과 성격', '꼼꼼히 읽기: 인기의 비결- ‘ 감성’과', '꼼꼼히 읽기: 인물로 본 연암의 여성 인식',
      '꼼꼼히 읽기: 전 고(典故)와 고사(故事', '꼼꼼히 읽기: 전 고(典故)와 고사(故훌', '선창 술래 좋다 강강술래 후 창 강강술래', '옛날 옛 시절 11 페}에',
      '옛날의 아름다운 만남 아득하여 쫓지 못하고 1', '용어 찾아보기', '인 명 챔 보기', '인영찾아보기', '작품찾아보기', '펴。으'}
...
def note(label):
    ...
    # The file names the model that wrote most of the 풀이; a note written later by
    # another one carries its own name, so the card credits the right writer.
    return {k:n[k] for k in ('label','definition','explanation','examples','basis')}|{'label':display_label(label),'citations':cites,'model':n.get('model') or _notes['model']}
